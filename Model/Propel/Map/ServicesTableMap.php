...
use Model\Propel\Services;
use Model\Propel\ServicesQuery;
...
class ServicesTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Model.Propel.Map.ServicesTableMap';
     ...
     */
    const OM_CLASS = '\\Model\\Propel\\Services';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Model.Propel.Services';
     ...
     */
    const COL_ID_SERVICE = 'services.id_service';

    /**
     * the column name for the title field
     */
    const COL_TITLE = 'services.title';
     ...
     */
    const COL_PRICE_VAT_EXCLUDED = 'services.price_vat_excluded';

    /**
     * the column name for the price_vat_included field
     */
    const COL_PRICE_VAT_INCLUDED = 'services.price_vat_included';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('IdService', 'Title', 'Description', 'PriceVatExcluded', 'PriceVatIncluded', ),
        self::TYPE_CAMELNAME     => array('idService', 'title', 'description', 'priceVatExcluded', 'priceVatIncluded', ),
        self::TYPE_COLNAME       => array(ServicesTableMap::COL_ID_SERVICE, ServicesTableMap::COL_TITLE, ServicesTableMap::COL_DESCRIPTION, ServicesTableMap::COL_PRICE_VAT_EXCLUDED, ServicesTableMap::COL_PRICE_VAT_INCLUDED, ),
        self::TYPE_FIELDNAME     => array('id_service', 'title', 'description', 'price_vat_excluded', 'price_vat_included', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('IdService' => 0, 'Title' => 1, 'Description' => 2, 'PriceVatExcluded' => 3, 'PriceVatIncluded' => 4, ),
        self::TYPE_CAMELNAME     => array('idService' => 0, 'title' => 1, 'description' => 2, 'priceVatExcluded' => 3, 'priceVatIncluded' => 4, ),
        self::TYPE_COLNAME       => array(ServicesTableMap::COL_ID_SERVICE => 0, ServicesTableMap::COL_TITLE => 1, ServicesTableMap::COL_DESCRIPTION => 2, ServicesTableMap::COL_PRICE_VAT_EXCLUDED => 3, ServicesTableMap::COL_PRICE_VAT_INCLUDED => 4, ),
        self::TYPE_FIELDNAME     => array('id_service' => 0, 'title' => 1, 'description' => 2, 'price_vat_excluded' => 3, 'price_vat_included' => 4, ),
        self::TYPE_NUM           => array(0, 1, 2, 3, 4, )
    );
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\Model\\Propel\\Services');
        $this->setPackage('Model.Propel');
        $this->setUseIdGenerator(true);
        // columns
        $this->addPrimaryKey('id_service', 'IdService', 'INTEGER', true, 8, null);
        $this->addColumn('title', 'Title', 'CHAR', true, null, null);
  ...
  array (
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(ServicesTableMap::COL_ID_SERVICE);
            $criteria->addSelectColumn(ServicesTableMap::COL_TITLE);
            $criteria->addSelectColumn(ServicesTableMap::COL_DESCRIPTION);
            $criteria->addSelectColumn(ServicesTableMap::COL_PRICE_VAT_EXCLUDED);
            $criteria->addSelectColumn(ServicesTableMap::COL_PRICE_VAT_INCLUDED);
        } else {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Model\Propel\Services) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(ServicesTableMap::DATABASE_NAME);
            $criteria->add(ServicesTableMap::COL_ID_SERVICE, (array) $values, Criteria::IN);
        }
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria->containsKey(ServicesTableMap::COL_ID_SERVICE) && $criteria->keyContainsValue(ServicesTableMap::COL_ID_SERVICE) ) {

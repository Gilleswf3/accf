...
abstract class Customers implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildTchat[] Collection to store aggregation of ChildTchat objects.
     */
    protected $collTchats;
    protected $collTchatsPartial;
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|ChildTchat[]
     */
    protected $tchatsScheduledForDeletion = null;
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->tchatsScheduledForDeletion !== null) {
                if (!$this->tchatsScheduledForDeletion->isEmpty()) {
                    \Model\Propel\TchatQuery::create()
                        ->filterByPrimaryKeys($this->tchatsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->tchatsScheduledForDeletion = null;
                }
            }

            if ($this->collTchats !== null) {
                foreach ($this->collTchats as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collTchats) {
                ...
                $result[$key] = $this->collTchats->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getTchats() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addTchat($relObj->copy($deepCopy));
                }
            }
    ...
    /**
     * Clears out the collTchats collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     ...
     * @see        addTchats()
     */
    public function clearTchats()
    {
        $this->collTchats = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collTchats collection loaded partially.
     */
    public function resetPartialTchats($v = true)
    {
        $this->collTchatsPartial = $v;
    }

    /**
     * Initializes the collTchats collection.
     *
     * By default this just sets the collTchats collection to an empty array (like clearcollTchats());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param      boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initTchats($overrideExisting = true)
    {
        if (null !== $this->collTchats && !$overrideExisting) {
            return;
        }

        $collectionClassName = TchatTableMap::getTableMap()->getCollectionClassName();

        $this->collTchats = new $collectionClassName;
        $this->collTchats->setModel('\Model\Propel\Tchat');
    }

    /**
     * Gets an array of ChildTchat objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildTchat[] List of ChildTchat objects
     * @throws PropelException
     */
    public function getTchats(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collTchatsPartial && !$this->isNew();
        if (null === $this->collTchats || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collTchats) {
                // return empty collection
                $this->initTchats();
            } else {
                $collTchats = ChildTchatQuery::create(null, $criteria)
                    ->filterByCustomers($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collTchatsPartial && count($collTchats)) {
                        $this->initTchats(false);

                        foreach ($collTchats as $obj) {
                            if (false == $this->collTchats->contains($obj)) {
                                $this->collTchats->append($obj);
                            }
                        }

                        $this->collTchatsPartial = true;
                    }

                    return $collTchats;
                }

                if ($partial && $this->collTchats) {
                    foreach ($this->collTchats as $obj) {
                        if ($obj->isNew()) {
                            $collTchats[] = $obj;
                        }
                    }
                ...
                $this->collTchats = $collTchats;
                $this->collTchatsPartial = false;
            }
        }

        return $this->collTchats;
    }

    /**
     * Sets a collection of ChildTchat objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $tchats A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildCustomers The current object (for fluent API support)
     */
    public function setTchats(Collection $tchats, ConnectionInterface $con = null)
    {
        /** @var ChildTchat[] $tchatsToDelete */
        $tchatsToDelete = $this->getTchats(new Criteria(), $con)->diff($tchats);


        $this->tchatsScheduledForDeletion = $tchatsToDelete;

        foreach ($tchatsToDelete as $tchatRemoved) {
            $tchatRemoved->setCustomers(null);
        }
    ...
    /**
     * Returns the number of related Tchat objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related Tchat objects.
     * @throws PropelException
     */
    public function countTchats(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collTchatsPartial && !$this->isNew();
        if (null === $this->collTchats || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collTchats) {
            ...
            if ($partial && !$criteria) {
                return count($this->getTchats());
            }

            $query = ChildTchatQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collTchats);
    }

    /**
     * Method called to associate a ChildTchat object to this object
     * through the ChildTchat foreign key attribute.
     *
     * @param  ChildTchat $l ChildTchat
     * @return $this|\Model\Propel\Customers The current object (for fluent API support)
     */
    public function addTchat(ChildTchat $l)
    {
        if ($this->collTchats === null) {
            $this->initTchats();
            $this->collTchatsPartial = true;
        ...
        if (!$this->collTchats->contains($l)) {
            $this->doAddTchat($l);

            if ($this->tchatsScheduledForDeletion and $this->tchatsScheduledForDeletion->contains($l)) {
                $this->tchatsScheduledForDeletion->remove($this->tchatsScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildTchat $tchat The ChildTchat object to add.
     */
    protected function doAddTchat(ChildTchat $tchat)
    {
        $this->collTchats[]= $tchat;
        $tchat->setCustomers($this);
    }

    /**
     * @param  ChildTchat $tchat The ChildTchat object to remove.
     * @return $this|ChildCustomers The current object (for fluent API support)
     */
    public function removeTchat(ChildTchat $tchat)
    {
        if ($this->getTchats()->contains($tchat)) {
            $pos = $this->collTchats->search($tchat);
            $this->collTchats->remove($pos);
            if (null === $this->tchatsScheduledForDeletion) {
                $this->tchatsScheduledForDeletion = clone $this->collTchats;
                $this->tchatsScheduledForDeletion->clear();
            }
            $this->tchatsScheduledForDeletion[]= clone $tchat;
            $tchat->setCustomers(null);
        }
     ...
     * Otherwise if this Customers is new, it will return
     * an empty collection; or if this Customers has previously
     * been saved, it will retrieve related Tchats from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Customers.
     ...
     * @return ObjectCollection|ChildTchat[] List of ChildTchat objects
     */
    public function getTchatsJoinEmployees(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildTchatQuery::create(null, $criteria);
        $query->joinWith('Employees', $joinBehavior);

        return $this->getTchats($query, $con);
    }
